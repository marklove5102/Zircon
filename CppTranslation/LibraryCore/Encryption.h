...
namespace Library
{
    ...
    class Encryption
    {
        private static uint8_t[] _cryptoKey = null;
        ...
        private static bool IsEncrypted(Stream stream)
        {
            ...
            return !new std::string[] { "Server.DBModels.", "Library.SystemMo", "Client.UserModel" }.Contains(stringToCompare);
};
        ...
        private static uint8_t[] GenerateIV(int32_t length)
        {
            uint8_t[] iv = new uint8_t[length];
        ...
        public static void SetKey(uint8_t[] databaseKey)
        {
