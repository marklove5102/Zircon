...
namespace Library
{
    ...
    class Stats
    {
        std::map<Stat, int32_t> Values; = new std::map<Stat, int32_t>();
    // IgnorePropertyPacket
    int32_t GetCount() const { return Values.Sum(pair => std::abs(pair.Value)); }
    // IgnorePropertyPacket
    int32_t operator[](Stat stat)
        ...
        {
            foreach (std::pair<Stat, int32_t> pair in stats.Values)
                ...
                switch (pair.Key)
                {
                    ...
                    case Stat.ItemReviveTime:
                        ...
                        if (this[pair.Key] == 0)
                            ...
                            this[pair.Key] = std::min(this[pair.Key], pair.Value);
                        break;
                    default:
                        this[pair.Key] += pair.Value;
                        break;
        ...
        public void Write(BinaryWriter writer)
        {
            ...
            foreach (std::pair<Stat, int32_t> pair in Values)
            {
                writer.Write((int32_t)pair.Key);
                writer.Write(pair.Value);
};

};

        public std::string GetTitle(Stat stat, bool groupSpellPower = true)
        {
        ...
        public std::string GetDisplay(Stat stat)
        {
            Type type = stat.GetType();
            ...
            switch (description.Mode)
            {
                case StatType.None:
                    return null;
                case StatType.Default:
                    return description.Title + ": " + std::string.Format(description.Format, this[stat]);
                case StatType.Min:
                    if (this[description.MaxStat] != 0) return null;

                    return description.Title + ": " + std::string.Format(description.Format, this[stat]);
                case StatType.Max:
                    return description.Title + ": " + std::string.Format(description.Format, this[description.MinStat], this[stat]);
                case StatType.Percent:
                    return description.Title + ": " + std::string.Format(description.Format, this[stat]/100D);
                case StatType.Text:
                    return description.Title;
                case StatType.Time:
                    if (this[stat] < 0)
                ...
                case StatType.SpellPower:
                    ...
                    return "Spell Power: " + std::string.Format(description.Format, this[description.MinStat], this[stat]);
                case StatType.AttackElement:
                ...
                case StatType.ElementResistance:
                    ...
                    foreach (Stat s in list)
                    {
                        ...
                        value += $"{description.Title} x" + std::abs(this[s]);
                        neecComma = true;
};
        ...
        public std::string GetFormat(Stat stat)
        {
            Type type = stat.GetType();
            ...
            {
                case StatType.Default:
                    return std::string.Format(description.Format, this[stat]);
                case StatType.Min:
                    return this[description.MaxStat] == 0 ? (std::string.Format(description.Format, this[stat])) : null;
                case StatType.Max:
                case StatType.SpellPower:
                    return std::string.Format(description.Format, this[description.MinStat], this[stat]);
                case StatType.Percent:
                    return std::string.Format(description.Format, this[stat] / 100D);
                case StatType.Text:
        ...
        public bool Compare(Stats s2)
        {
            ...
            foreach (std::pair<Stat, int32_t> value in Values)
        ...
        public int32_t GetWeaponElementValue()
        {
        ...
        public int32_t GetElementValue(Element element)
        {
            switch (element)
            {
                case Element.Fire:
        ...
        public int32_t GetAffinityValue(Element element)
        {
            switch (element)
            {
                case Element.Fire:
        ...
        public int32_t GetResistanceValue(Element element)
        {
            switch (element)
            {
                case Element.Fire:
    ...
    class StatDescription : Attribute
    {
        std::string Title;
        std::string Format;
        StatType Mode;
