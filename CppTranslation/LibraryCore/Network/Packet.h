...
namespace Library::Network
{
    ...
    {
        private static readonly std::vector<Type> Packets;
        private static readonly std::unordered_map<Type, Action<object, BinaryWriter>> TypeWrite;
        private static readonly std::unordered_map<Type, Func<BinaryReader, object>> TypeRead;

        public static bool IsClient { get; set; }

        public Type PacketType;

        public int32_t Length;
        public bool ObserverPacket = true;

        static Packet()
        {
            Packets = new std::vector<Type>();
            ...
            Packets.Sort((x1, x2) =>
            {
                ...
                if (std::string.Compare(x1.Namespace, @"Library.Network.GeneralPackets", StringComparison.Ordinal) == 0) //We want General Packets shifted To the top.
                    return -1;

                if (std::string.Compare(x2.Namespace, @"Library.Network.GeneralPackets", StringComparison.Ordinal) == 0) //We want General Packets shifted To the top.
                    return 1;
            ...
            TypeWrite = new std::unordered_map<Type, Action<object, BinaryWriter>>
            {
                ...
                [typeof(Byte[])] = (v, w) =>
                {
                    ...
                    w.Write(v != null ? (Byte[]) v : new uint8_t[0]);
                },
                ...
                [typeof(String)] = (v, w) => w.Write((String) v ?? std::string.Empty),
            ...
            TypeRead = new std::unordered_map<Type, Func<BinaryReader, object>>
            {
        ...
        public static Packet ReceivePacket(uint8_t[] rawBytes, out uint8_t[] extra)
        {
            extra = rawBytes;
            ...
            int32_t length = rawBytes[3] << 24 | rawBytes[2] << 16 | rawBytes[1] << 8 | rawBytes[0];

            if (length > rawBytes.Length) return null;

            extra = new uint8_t[rawBytes.Length - length];
        ...
        public uint8_t[] GetPacketBytes()
        {
            ...
            {
                writer.Write((int16_t)Packets.IndexOf(GetType()));
